/* 체인법으로 구현한 해시(소스) */
/* 문자열 해시값 https://kirkim.github.io/c/2021/04/22/hash4.html */ 
#include <stdio.h>
#include <stdlib.h>
#include "Member.h"
#include "ChainHash.h"
...
Node* Search(const ChainHash* h, const Member* x)
{
	...
	while (p != NULL)
	{
		if (MemberNameCmp(&p->data, x) == 0)		/* 검색 성공 */
			return p;
		p = p->next;					/* 다음 노드를 선택 */
	}
...
int Add(ChainHash* h, const Member* x)
{
	...
	while (p != NULL)
	{
		if (MemberNameCmp(&p->data, x) == 0)		/* 이 키는 이미 등록됨 */
			return 1;					/* 추가 실패 */
		p = p->next;					/* 다음 노드를 선택 */
	}
...
int Remove(ChainHash* h, const Member* x)
{
	...
	while (p != NULL) {
		if (MemberNameCmp(&p->data, x) == 0) {		/* 찾으면 */
			*pp = p->next;
			free(p);					/* 해제 */
			return 0;					/* 삭제 성공 */
		}
