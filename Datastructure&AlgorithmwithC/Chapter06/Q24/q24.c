...
/*--- 도수 정렬 함수(배열의 요솟값은 min 이상 max 이하) ---*/
void counting2(int a[], int n, int min, int max)
{
    int* f = calloc(max - min + 1, sizeof(int));      // 누적 도수
    int* b = calloc(n, sizeof(int));      // 작업용 목적 배열
...
int main(void)
{
    ...
    counting2(x, nx, min, max);       // 배열 x를 도수 정렬
    puts("오름차순으로 정렬했습니다.");
...
//{
//    int* f = calloc(max - min + 2, sizeof(int));    // 누적 도수 max - min + 2??? 이건 좀
//    int* b = calloc(n, sizeof(int));                // 작업용 목적 배열
//
