...
/*--- 꼬리노드를 삭제 ---*/
void RemoveRear(List* list) // tail node가 배열의 꼬리 노드일 때 제거한다면 왜 max에서 하나를 빼지 않지? 안 빼도 그자리가 delete node이니 GetIndex할 때 max를 증가시키지 않고 추가하면 된다.
{
	if (list->head != NULL) {
...
void Purge(List *list, int compare(const Member *x, const Member *y))
{
	...
	while (ptr != NULL)
	{
		...
		while (list->n[pre].next != NULL)
		{
			ptr2 = list->n[pre].next;
			if (compare(&list->n[ptr].data, &list->n[ptr2].data) == 0)
			{
